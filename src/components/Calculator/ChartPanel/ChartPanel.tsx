...
const ChartPanel: React.FC = () => {
  ...
  const [dateLowerThanTodayError, setDateLowerThanTodayError] =
    useState<boolean>(false);
  const [isCorrect, setIsCorrect] = useState<boolean>(false);
  const [emptyDate, setEmptyDate] = useState<boolean>(false);

  const dateInChartErrorMessage = dateError ? (
    <p className="mt-4 text-center font-bold text-red-700">
      Date already in chart
    </p>
  ) : null;
  const weightErrorMessage = weightError ? (
    <p className="mt-4 text-center font-bold text-red-700">
      Weight must have positive value
    </p>
  ) : null;
  const emptyDateMessage = emptyDate ? (
    <p className="mt-4 text-center font-bold text-red-700">
      Date input left empty
    </p>
  ) : null;
  const dateLowerThanTodayErrorMessage = dateLowerThanTodayError ? (
    <p className="mt-4 text-center font-bold text-red-700">
  ...
  const handleSubmit = (event: React.SyntheticEvent) => {
    ...
    const chceckData: DataProps | undefined = data.find(
      (elem) => elem.date === date
    );
    const correctDate: boolean =
      parseInt(`${date[date.length - 2]}${[date.length - 1]}`) <
      new Date().getDate()
        ? false
        : true;
    if (
      weight > 0 &&
      chceckData === undefined &&
      correctDate &&
      date.length === 10
    ) {
      setData([
    ...
    <section className="w-full h-full">
      <div className="h-full w-full flex flex-col">
        <div className="h-3/5 my-auto justify-around">
          <h2 className="text-dark-blue w-4/5 mx-auto mb-2 text-center font-bold text-2xl">
            Weight chart
          </h2>
        ...
        <div className="h-1/3 flex my-auto justify-around">
          <form
            ...
            className="shadow text-dark-blue w-4/5 rounded border-2 p-4 border-extra-light-blue border-solid 2xl:overflow-auto lg:overflow-y-scroll"
          >
