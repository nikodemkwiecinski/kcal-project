...
const Navbar: React.FC<Props> = ({
  ...
  let navIcons: Array<JSX.Element> | null | undefined = userLoged
    ? icons.map((elem) => (
        <ListElement
  ...
  const logIcon: JSX.Element = !userLoged ? (
    <li
      ...
      onClick={() => setBlurToogle((prevState) => !prevState)}
    >
      {ICONS[0].jsx}
  ...
  return (
    <>
      <nav
        className={`flex flex-col bg-light-blue 2xl:w-64 xl:w-40 lg:w-32 h-screen sticky top-0 ${
          blurToogle ? "blur-sm" : ""
        }`}
      >
        ...
        <span className="text-white 2xl:text-4xl xl:text-2xl lg:text-xl mb-4">
          <div className="flex justify-between bottom-4">
            <FontAwesomeIcon
              ...
              className="2xl:ml-16 xl:ml-8 lg:ml-4"
            />
            <FontAwesomeIcon
              icon={brands("instagram")}
              className="2xl:mr-16 xl:mr-8 lg:mr-4"
            />
          </div>
        </span>
      </nav>
